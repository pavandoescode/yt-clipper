...
function ClipCard({ clip, onSaveToggle }) {
    ...
    // Convert timestamp to seconds for YouTube URL (handles HH:MM:SS, MM:SS, or SS)
    ...
    // Format timestamp for display (removes 00: prefix if hours are zero)
    const formatTimestamp = (ts) => {
        if (!ts) return '';
        // If format is 00:MM:SS, show as MM:SS
    ...
    const handleOpenClick = () => {
        // Save this clip as last opened
        localStorage.setItem('lastOpenedClipId', clip._id);
        setIsLastOpened(true);
        // Notify other clips to update their state
                ...
                <div className="clip-inputs">
                    ...
                    <div style={{ display: 'flex', gap: '12px' }}>
                        <input
                            ...
                            onChange={(e) => handleTextChange(setThumbnailTopText, e.target.value)}
                            className="clip-input"
                            style={{ flex: 1 }}
                        />
                        <input
                            ...
                            onChange={(e) => handleTextChange(setThumbnailMainText, e.target.value)}
                            className="clip-input"
                            style={{ flex: 1 }}
                        />
                    </div>
